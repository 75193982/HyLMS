...
<div class="page-content">
		...
		<div class="mng">
		   ...
		   <div class="table-item">
		     ...
		     <div class="row newrow">
		       ...
		       <div class="col-xs-5">
			       <div class="form-contr">
			         <input class="form-control" id="all-amount" type="text" placeholder="请输入台数" disabled="disabled" />
			       </div>
		       </div>
		       <div class="col-xs-1 pd-2">
				       <div class="lab-tit">
		     ...
		     <div class="row row-btn-tit" id="carDetail">
		       ...
		       <div class="col-xs-2">
				  <div class="form-contr-1">
				     <a class="form-btn-1" onclick="addNewCar();"><i class="icon-plus-sign" style="display: inline-block;width: 20px;"></i>商品车</a>
		     ...
		     <div class="col-xs-3"></div>
		       <div class="col-xs-1">
			       <div class="form-contr">
			          <a class="savebtn" onclick="doupdate();"><i class="icon-save" style="display: inline-block;width: 20px;"></i>保存</a>
			       </div>
		       </div>
	...
	<!-- 新增配件信息--Modal begin-->
	<div class="modal fade" id="modal-addPart" tabindex="-1" role="dialog">
...
<script type="text/javascript">
	            	...
	            	if(data.data!=null && data.data!=''){
	            		if(data.data.length>0){
	            			for(var i=0;i<data.data.length;i++){
	                			html +='<option value='+data.data[i]['no']+' data-driverId='+data.data[i]['driverId']+' data-driverName='+data.data[i]['driverName']+' data-id='+data.data[i]['id']+' data-mobile='+data.data[i]['mobile']+'>'+data.data[i]['no']+'['+data.data[i]['status']+']</option>';
	                		}
	            		}
	            	}
	            	$('#carNumber').html(html);
  ...
  /* 编辑时绑定4s店 */
  function choose4S(index,carShopId){
	  $.ajax({  
	        url: '${ctx}/operationMng/scheduleMng/getCarShopList',  
	        type: "post",  
	        contentType : "application/json;charset=UTF-8",
			...
			success : function(data) {
				if (data && data.code == 200) {
					...
					if(data.data['sendTime']!=''&&data.data['sendTime']!=null){
						$('#sendTime').val(jsonForDateFormat(data.data['sendTime']));
					}else{
						$('#sendTime').val('');
					}
					if(data.data['receiveTime']!=''&&data.data['receiveTime']!=null){
						$('#receiveTime').val(jsonForDateFormat(data.data['receiveTime']));
					}else{
						$('#receiveTime').val('');
					}
					$('#all-amount').val(data.data['amount']);
					...
					if(data.data['preList']!=null && data.data['preList'].length>0){
						$('#prepayCash').val(data.data['preList'][0]['prepayCash']);
						$('#bankName').val(data.data['preList'][0]['bankName']);
						$('#bankAccount').val(data.data['preList'][0]['bankAccount']);
						$('#oilCardNo').val(data.data['preList'][0]['oilCardNo']);
						$('#oilAmount').val(data.data['preList'][0]['oilAmount']);
					}
						...
						for(var i=0;i<data.data['detailList'].length;i++){
							...
							if(carInfo!=null && carInfo.length>0){
								for(var j=0;j<carInfo.length>0;j++){
									...
									if(carShopName!=null && carShopName!=""){
										carHtmlItem+='<tr><td data-id="'+carInfo[j]['id']+'">'+carInfo[j]['vin']+'</td><td>'+carInfo[j]['model']+'</td>'
										   +'<td>'+carInfo[j]['color']+'</td><td>'+jsonDateFormat(carInfo[j]['insertTime'])+'</td>'
										   +'<td>'+carInfo[j]['waybillNo']+'</td><td>'+carShopName+'</td>'
										   +'<td>'+carInfo[j]['brand']+'</td><td><a class="deleteBtn" onclick="deleteCar(this)">删除</a></td></tr>';
									}else{
										carHtmlItem+='<tr><td data-id="'+carInfo[j]['id']+'">'+carInfo[j]['vin']+'</td><td>'+carInfo[j]['model']+'</td>'
										   +'<td>'+carInfo[j]['color']+'</td><td>'+jsonDateFormat(carInfo[j]['insertTime'])+'</td>'
								...
								carHtmlItem='<tr><td colspan="8">暂无车辆信息</td></tr>'
								carHtml='<table class="carList table table-striped table-bordered table-hover">'
							        +'<thead><tr><th>车架号</th><th>车型</th><th>颜色</th><th>入库时间</th><th>运单编号</th><th>经销单位</th><th>品牌</th><th>操作</th></tr></thead>'
							        +'<tbody>'+carHtmlItem+'</tbody></table>';
							}
							...
							if(carAttachmentInfo!=null && carAttachmentInfo.length>0){
								for(var k=0;k<carAttachmentInfo.length>0;k++){
									carAttachmentHtmlItem+='<tr><td data-id="'+carAttachmentInfo[k]['id']+'">'+carAttachmentInfo[k]['attachmentName']+'</td>'
										   +'<td>'+carAttachmentInfo[k]['position']+'</td>'
										   +'<td>'+carAttachmentInfo[k]['count']+'</td>'
										   +'<td>'+carAttachmentInfo[k]['outCount']+'</td><td><a class="deleteBtn" onclick="deletePart(this)">删除</a></td></tr>';
									
								}
							}else{
								carAttachmentHtmlItem='<tr><td colspan="5">暂无配件信息</td></tr>'
							}
						        ...
						        if(data.data['detailList'][i]['carShopId']!=null && data.data['detailList'][i]['carShopId']!=''){
						        	if(i==0){
						        		detailListHtml+='<div class="border-b-ff9a00 detailList" id="detailList'+i+'">'
										  +'<div class="row newrow"><div class="col-xs-1 pd-2"><div class="lab-tit"><span class="red">*</span><label>经销单位:</label></div></div>'
							              +'<div class="col-xs-3"><div class="form-contr"><select class="form-control carShopId" id="carShop'+i+'" onChange="change4S(this,'+i+');"></select></div></div>'
							              +'<div class="col-xs-1 pd-2"><div class="lab-tit"><span class="red">*</span><label>台数:</label></div></div>'
							              +'<div class="col-xs-3"><div class="form-contr"><input class="form-control initAmount" id="amount'+i+'" type="text" value="'+data.data['detailList'][i]['amount']+'" placeholder="请输入台数" disabled="disabled"></div></div>'
							              +'<div class="col-xs-1 pd-2"><div class="lab-tit"><label>详细指令:</label></div></div>'
							              +'<div class="col-xs-3"><div class="form-contr"><input class="form-control" id="detailMark'+i+'" type="text" value="'+data.data['detailList'][i]['mark']+'" placeholder="请输入详细指令"></div></div></div>'
							              
							              +'<div class="row newrow"><div class="col-xs-1 pd-2"><div class="lab-tit"><label><span class="red">*</span>始发省:</label></div></div>'
									       +'<div class="col-xs-3"><div class="form-contr"><select class="form-control carShopId" id="startProvince'+i+'" onchange="createCities(this,\'startAddress'+i+'\')" ></select></div></div>'
									       +'<div class="col-xs-1 pd-2"><div class="lab-tit"><label><span class="red">*</span>始发地:</label></div></div>'
									       +'<div class="col-xs-3"><div class="form-contr"><select class="form-control carShopId" id="startAddress'+i+'" ><option value="---请选择市县---">---请选择市县---</option></select></div></div>'
									       +'<div class="col-xs-1 pd-2"><div class="lab-tit"><label><span class="red">*</span>目的省:</label></div></div>'
									       +'<div class="col-xs-3"><div class="form-contr"><select class="form-control carShopId" id="endProvince'+i+'" onchange="createCitiesEnd(this,\'endAddress'+i+'\')" ></select></div></div></div>'
									       +'<div class="row newrow"><div class="col-xs-1 pd-2"><div class="lab-tit"><label><span class="red">*</span>目的地:</label></div></div>'
									       +'<div class="col-xs-3"><div class="form-contr"><select class="form-control carShopId" id="endAddress'+i+'" ><option value="---请选择市县---">---请选择市县---</option></select></div></div></div>'
									      
							              +'<div class="row newrow"><div class="col-xs-1 pd-2"><div class="lab-tit"><label>车辆信息:</label></div></div>'
							              +'<div class="col-xs-11">'
							              +'<div class="form-contr"><a class="form-btn" id="addCarItem'+i+'" onclick="bindCar('+i+')"><i class="icon-plus-sign" style="display: inline-block;width: 20px;"></i>新增车辆信息</a></div>'
							              +'<div class="newCarDetail-item newCarDetail" data-id="'+i+'"  id="newCarDetail'+i+'">'+carHtml+'</div></div></div>'
							              +'<div class="row newrow"><div class="col-xs-1 pd-2"><div class="lab-tit"><label>配件信息:</label></div></div>'
							              +'<div class="col-xs-11">'
							              +'<div class="form-contr"><a class="form-btn" id="addPartItem'+i+'" onclick="bindPart('+i+')"><i class="icon-plus-sign" style="display: inline-block;width: 20px;"></i>新增配件信息</a></div>'
							              +'<div class="newPartDetail-item" id="newPartDetail'+i+'">'+carAttachmentHtml+'</div></div></div></div>';
						        	}else{
						        		detailListHtml+='<div class="border-b-ff9a00 detailList" id="detailList'+i+'">'
									...
									if(i==0){
										detailListHtml+='<div class="border-b-ff9a00 detailList" id="detailList'+i+'">'
										  +'<div class="row newrow"><div class="col-xs-1 pd-2"><div class="lab-tit"><span class="red">*</span><label>经销单位:</label></div></div>'
							              +'<div class="col-xs-3"><div class="form-contr"><p class="carShopId-item form-control">二手车</p></div></div>'
							              +'<div class="col-xs-1 pd-2"><div class="lab-tit"><span class="red">*</span><label>台数:</label></div></div>'
							              +'<div class="col-xs-3"><div class="form-contr"><input class="form-control initAmount" id="amount'+i+'" type="text" value="'+data.data['detailList'][i]['amount']+'" placeholder="请输入台数" disabled="disabled"></div></div>'
							              +'<div class="col-xs-1 pd-2"><div class="lab-tit"><label>详细指令:</label></div></div>'
							              +'<div class="col-xs-3"><div class="form-contr"><input class="form-control" id="detailMark'+i+'" type="text" value="'+data.data['detailList'][i]['mark']+'" placeholder="请输入详细指令"></div></div></div>'
									      ...
									      +'<div class="row newrow"><div class="col-xs-1 pd-2"><div class="lab-tit"><label>车辆信息:</label></div></div>'
							              +'<div class="col-xs-11">'
							              +'<div class="form-contr"><a class="form-btn" id="addCarItem'+i+'" onclick="bindOldCar('+i+')"><i class="icon-plus-sign" style="display: inline-block;width: 20px;"></i>新增车辆信息</a></div>'
							              +'<div class="newCarDetail-item newCarDetail" data-id="'+i+'" id="newCarDetail'+i+'">'+carHtml+'</div></div></div>'
							              +'<div class="row newrow"><div class="col-xs-1 pd-2"><div class="lab-tit"><label>配件信息:</label></div></div>'
							              +'<div class="col-xs-11">'
							              +'<div class="form-contr"><a class="form-btn" id="addPartItem'+i+'" onclick="bindOldPart('+i+')"><i class="icon-plus-sign" style="display: inline-block;width: 20px;"></i>新增配件信息</a></div>'
							              +'<div class="newPartDetail-item" id="newPartDetail'+i+'">'+carAttachmentHtml+'</div></div></div></div>';
									}else{
										detailListHtml+='<div class="border-b-ff9a00 detailList" id="detailList'+i+'">'
										  +'<div class="row newrow"><div class="col-xs-10 pd-2"></div><div class="col-xs-2"><div class="form-contr-1"><a class="delete-detail fr" onclick="removeDetail(this,'+i+');"><i class="icon-minus-sign" style="display: inline-block;width: 20px;"></i>删除</a></div></div></div>'
							              ...
							              +'<div class="col-xs-11">'
							              +'<div class="form-contr"><a class="form-btn" id="addPartItem'+i+'" onclick="bindOldPart('+i+')"><i class="icon-plus-sign" style="display: inline-block;width: 20px;"></i>新增配件信息</a></div>'
							              +'<div class="newPartDetail-item" data-id="'+i+'" id="newPartDetail'+i+'">'+carAttachmentHtml+'</div></div></div></div>';
									}
									
								}
						        
						        choose4S(i,data.data['detailList'][i]['carShopId']);
						        
							}

						}else{
							detailListHtml='<div class="border-b-ff9a00 detailList" id="detailList0"><div class="row newrow"><div class="col-xs-12"><p class="form-control no-border t-c">没有详细信息！</p></div></div></div>';
						}
					...
					if(data.data['detailList'].length > 0)
					{
						for(var j=0;j<data.data['detailList'].length;j++)
						{
							var startProvince=data.data['detailList'][j]['startProvince'];
					        var startAddress=data.data['detailList'][j]['startAddress'];
					        var targetProvince=data.data['detailList'][j]['targetProvince'];
					        var targetCity=data.data['detailList'][j]['targetCity'];
					        ...
					        //创建省份  
					          createProvinces();
					          createProvincesEnd();
					          loadcitys(startProvince,'startAddress'+j);
					          loadcitysEnd(targetProvince,'endAddress'+j);
					          $('#startProvince'+j).val(startProvince);
					          $('#startAddress'+j).val(startAddress);
					          $('#endProvince'+j).val(targetProvince);
					          $('#endAddress'+j).val(targetCity);
						}
					}
		  ...
		  callback: function(result){
			  if(result){
			   var html='<div class="border-b-ff9a00 detailList" id="detailList'+index+'">'
				       ...
				       +'<div class="col-xs-3"><div class="form-contr"><input class="form-control initAmount" id="amount'+index+'" type="text" placeholder="请输入台数" disabled="disabled" value="0"/></div></div>'
				       +'<div class="col-xs-1 pd-2"><div class="lab-tit"><label>详细指令:</label></div></div>'
...
/* 二手车配件多选框选择 */
function checkChooseOld(size,all){
	   var num = size; //选中总个数 
	   var chk = all; //选项总个数  
	    if(num==chk && num>0){//全选 
...
/* 商品车多选框选择 */
function checkChooseCar(size,all){
	   var num = size; //选中总个数
	   var chk = all; //选项总个数 
	    if(num==chk && num>0){//全选 
...
/* 二手车多选框选择 */
function checkChooseOldCar(size,all){
	   var num = size;//选中总个数 
	   var chk = all; //选项总个数
	    if(num==chk && num>0){//全选 
		  ...
		  callback: function(result){
			  if(result){
					...
					for(i=0;i<table.children('tr.selected').length;i++){
						...
						obj.model=tr.find('td').eq(3).html();
						obj.color=tr.find('td').eq(4).html();
						obj.insertTime=tr.find('td').eq(5).html();
						obj.waybillNo=tr.find('td').eq(6).html();
						obj.carshop=$("#carShop"+index+"").find("option:selected").text();
						if(tr.find('td').eq(7).html()!='' && tr.find('td').eq(7).html()!=null){
