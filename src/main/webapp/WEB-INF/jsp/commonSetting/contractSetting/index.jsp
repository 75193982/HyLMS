...
<div class="page-content">
	...
	<div class="page-content">
		...
		<div class="searchbox col-xs-12">
		    <label class="title" style="margin-left: 18px;">类&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;型：</label>
		   <select id="fom_type" class="form-box" style="width: 234px;">
		   <option value="">请选择类型</option>
		   <option value="0">员工合同</option>
		   <option value="1">外协单位合同</option>	  
		   	</select>
		   	<label class="title" style="margin-left: -6px;">合同编号：</label>
		   <input id="fom_code" class="form-box" type="text" placeholder="请输入合同编号" style="width:234px;"/>
				   ...
				   <div>
					  <div class="widget-box dia-widget-box">
							...
							<div class="add-item col-xs-12">
							 ...
							 <hr class="tree"></hr>
							 <div class="add-item col-xs-12">
							     <label class="title col-xs-3"><span class="red">*</span>主体编号：</label>
							     <div class="col-xs-9" style="margin-bottom: 10px;">
							    <select id="mainId" class="form-control" >							  
							    </select>
							    </div>							   
							  </div>							  
							 <hr class="tree"></hr>
							  <div class="add-item col-xs-12">
							     <label class="title col-xs-3"><span class="red">*</span>合同编码 ：</label>
							     <div class="col-xs-9" style="margin-bottom: 10px;">
							     <input class="form-control" id="code" type="text" placeholder="请输入合同编码  "/>
							     <!-- <input class="form-control" id="brand" type="text" placeholder="请输入品牌  "/> -->
							      </div>	
							  </div>
							  <hr class="tree"></hr>
							  <div class="add-item col-xs-12">
							     <label class="title col-xs-3"><span class="red">*</span>开始时间：</label>
							     <div class="input-group input-group-sm col-xs-9" style="margin-bottom: 10px;">
							   ...
							   <hr class="tree"></hr>
							  <div class="add-item col-xs-12">
							     <label class="title col-xs-3"><span class="red">*</span>结束时间 ：</label>
							     <div class="input-group input-group-sm col-xs-9" style="margin-bottom: 10px;">
							   ...
							   <!-- <hr class="tree"></hr>
							  <div class="add-item col-xs-12">
							     <label class="title col-xs-3">到期提醒时间 ：</label>
							     <div class="input-group input-group-sm col-xs-9" style="margin-bottom: 10px;">
...
function init(){
	...
	var myTable = $('#detailtable').DataTable({
		 ...
		 "sAjaxSource": "${ctx}/commonSetting/contractSetting/getListData" , //获取数据的ajax方法的URL							 
		 ordering: false,	
			"oLanguage": {
			    	 ...
			    	 render: function (data, type, row, meta) {
			    		 if(row.status=='0'){
			    			 return '<a class="table-edit" onclick="dosubmit('+ row.id +')">提交</a>'
					           ...
					           +'<a class="table-upload" onclick="doupload(\''+ row.id +'\',\''+ row.filePath +'\')">附件上传</a>';
			    		 }else if(row.filePath==''||row.filePath==null){
			    			 return '<a class="table-edit" onclick="doview('+ row.id +')">查看</a>'; 
			    		 }else{
			    			 return '<a class="table-edit" onclick="doview('+ row.id +')">查看</a>'
					           +'<a class="table-upload"  href="${ctx}'+row.filePath+'" target="_blank">附件查看</a>' ;  
			    		 }
		                }	       
		    	} 
		      ],
		 ...
		 ordering: false,	
			"oLanguage": {
				...
				columns: [{ data: "rownum" ,"width":"4%"},
						    {data: "type","width":"8%"}, 
						    {data: "mainName","width":"8%"},
						    {data: "code","width":"8%"},
						    {data: "startTime","width":"10%"},
						    {data: "endTime","width":"10%"},
						    {data: "mark","width":"10%"},
						    {data: "status","width":"8%"},
						   /*  {data: "noticeTime","width":"10%"},	 */    
						    {data: null,"width":"15%"}],
						    columnDefs: [{
								 //入职时间
								 targets: 1,
	...
	bootbox.confirm({ 
		  ...
		  callback: function(result){
			  if(result){
				  $.ajax({
						type : 'GET',
						url : "${ctx}/commonSetting/contractSetting/delete/"+id,
						contentType : "application/json;charset=UTF-8",
						dataType : 'JSON',
						success : function(data) {
							if (data && data.code == 200) {
...
function doadd(){
	...
	$('#editBtn').hide();
	$('#viewBtn').hide();
	$('#myModalLabel').html('合同新增');	
	$('#modal-info').modal('show');
	$("#type").attr("disabled",false);
	$("#mainId").attr("disabled",false);
	$("#code").attr("disabled",false);
	$("#startTime").attr("disabled",false);
	$("#endTime").attr("disabled",false);
	/* $("#noticeTime").attr("disabled",false); */
	...
	bindMainId();//绑定主体编号
		...
		var type=$(this).children('option:selected').val();		
		 if(type!=''){
			 $.ajax({
					type : 'GET',
					url : "${ctx}/commonSetting/contractSetting/getMainIdList/"+type,
					contentType : "application/json;charset=UTF-8",
					dataType : 'JSON',
					success : function(data) {
						if (data && data.code == 200) {
							//console.info(JSON.stringify(data.data));
							var html ='<option value="">请选择主体</option>';
...
function save(){
	...
	if(type==''|| type==null){
		...
		bootbox.alert('主体编号不能为空！');
		return;
	}if(code==''|| code==null){
		bootbox.alert('合同编号不能为空！');
		return;
	...
	bootbox.confirm({ 
		  ...
		  callback: function(result){
			  if(result){
					$.ajax({
						type : 'POST',
						url : '${ctx}/commonSetting/contractSetting/save',
						data : JSON.stringify({
							type : type,				
							mainId : mainId,
							code   : code,
...
function doedit(id){	
	...
	$.ajax({
		type : 'GET',
		url : "${ctx}/commonSetting/contractSetting/getDetailData/"+id,
		contentType : "application/json;charset=UTF-8",
		dataType : 'JSON',
		success : function(data) {
			if (data && data.code == 200) {
				...
				$.ajax({
					type : 'GET',
					url : "${ctx}/commonSetting/contractSetting/getMainIdList/"+data.data.type,
					contentType : "application/json;charset=UTF-8",
					dataType : 'JSON',
					success : function(data) {
						if (data && data.code == 200) {
	...
	bootbox.confirm({ 
		  ...
		  callback: function(result){
			  if(result){
					$.ajax({
						type : 'POST',
						url : '${ctx}/commonSetting/contractSetting/update',
						data : JSON.stringify({
							id : id,
							type : type,				
							mainId : mainId,
...
function doupload(id,filePath){
	...
	$("#inputfile").uploadify({
       ...
       /*  'fileTypeExts':'*.zip;*.rar', */
        ...
        //处理上传文件的服务类
        'uploader':'${ctx}/upload/saveFile?type=contract',
        /* //上传文件个数限制
        'uploadLimit': 1, */
        //上传按钮内容显示文本
        'buttonText':'上传',
	...
	$.ajax({
		...
		success : function(data) {
			if (data && data.code == 200) {
				...
				$("#mainId").attr("disabled",true);
				$("#code").attr("disabled",true);
				$("#startTime").attr("disabled",true);
				$("#endTime").attr("disabled",true);
				/* $("#noticeTime").attr("disabled",true); */
		...
		bootbox.confirm({ 
		  ...
		  callback: function(result){
			  if(result){
				  $.ajax({
						type : 'POST',
						url : '${ctx}/commonSetting/contractSetting/updateFilePath',
						data : JSON.stringify({
							id : id,
							filePath : filepath
						}),
	...
	bootbox.confirm({ 
		  ...
		  callback: function(result){
			  if(result){
				  $.ajax({
						type : 'GET',
						url : "${ctx}/commonSetting/contractSetting/submit/"+id,
						contentType : "application/json;charset=UTF-8",
						dataType : 'JSON',
						success : function(data) {
							if (data && data.code == 200) {
