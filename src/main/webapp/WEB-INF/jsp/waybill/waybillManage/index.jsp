				   ...
				   <div>
					  <div class="widget-box dia-widget-box">
							...
							<hr class="tree"></hr>
							 ...
							 <hr class="tree"></hr>
							   <div class="add-item col-xs-12">
							     <label class="title col-xs-3"><span class="red">*</span>始发省：</label>
							     <div class="col-xs-9">  
							     <select id="startProvince_first" class="form-control">							          
			                     </select>
			                    </div>
							    </div>	
							  <hr class="tree"></hr>
							   <div class="add-item col-xs-12">
							     ...
							     <div class="col-xs-9"> 
							     <select id="startAddress_first" class="form-control">							          
			                     </select>
							     </div>
							    </div>	
							     <hr class="tree"></hr>
							     ...
							     <div class="col-xs-9"> 
							     <select id="targetProvince_first" class="form-control">							          
			                     </select>
							     </div>
							    </div>				
							  <hr class="tree"></hr>
							   <div class="add-item col-xs-12">
							     ...
							     <div class="col-xs-9"> 
							     <select id="targetAddress_first" class="form-control">							          
			                     </select>
							     </div>
							    </div>				
							  <!-- <hr class="tree"></hr>
...
<script src="${ctx}/staticPublic/js/cityStartEnd.js"></script>
			    	 ...
			    	 render: function (data, type, row, meta) {	
			    		 if(row.status=='0'){
			    			 return '<a class="table-edit" onclick="doedit('+ row.id +')">编辑</a>'
					           ...
					           +'<a class="table-bingcars" onclick="dobindcar('+ row.id +')">绑定商品车</a>'
					           +'<a class="table-audit" style="margin-top:1px;" onclick="dobindattachment('+ row.id +')">绑定配件</a>'
					           +'<a class="table-edit" style="margin-top:1px;" onclick="dosubmit('+ row.id +')">提交</a>';	
			    		 }else{
...
function clear(){
	...
	document.getElementById("startProvince_first").options.length = 0;
	document.getElementById("startAddress_first").options.length = 0;
	//$('#startAddress_next').val('');
	//$('#targetAddress_first').val('');
	//$('#targetAddress_next').val('');
	//$('#targetProvince_first').val('');
	document.getElementById("targetProvince_first").options.length = 0;
	document.getElementById("targetAddress_first").options.length = 0;
	$('#distance').val('');	
	$('#inputfile').val('');
	$('#filename').html('');
	$('#filename_hidden').val('');
...
function save(){
	...
	var startProvince = $("#startProvince_first").val();// ww 2017.2.10
	...
	if(startProvince == '' || startProvince == null || startProvince == '---请选择省份---')
	{
		bootbox.alert('始发省不能为空！');
		return;
	}
	if(startAddress_first=='' || startAddress_first==null || startAddress_first == '---请选择市县---'){
		bootbox.alert('始发地不能为空！');
		return;
	}
	if(targetProvince_first=='' || targetProvince_first==null || targetProvince_first == '---请选择省份---'){
		bootbox.alert('目的省不能为空！');
		return;
	}
	if(targetAddress_first=='' || targetAddress_first==null || targetAddress_first== '---请选择市县---'){
	...
	$.ajax({
		...
		success : function(data) {
			if (data && data.code == 200) {
				...
				//ww 2017.2.10
				loadcitys(data.data.startProvince);
				$("#startAddress_first").val(data.data.startAddress);
				$("#startProvince_first").val(data.data.startProvince);
				loadcitysEnd(data.data.targetProvince);
...
function update(){
	...
	var receiveUserMobile=$("#receiveUserMobile").val(); 
	//ww
	var startProvince=$("#startProvince_first").val(); 
	var startAddress_first=$("#startAddress_first").val(); 
	//var startAddress_next=$("#startAddress_next").val(); 
	var targetAddress_first=$("#targetAddress_first").val();
	var targetProvince_first=$("#targetProvince_first").val();
