...
<div class="page-content">
	...
	<div class="page-content">
		<div class="searchbox col-xs-12">
			<label class="title" style="float: left;height: 30px;line-height: 30px;">操作时间：</label>
		    <div class="input-group input-group-sm" style="float: left;width: 234px;margin-right:15px; margin-left: 2px;height: 30px;line-height: 30px;">
				<input class="form-control" id="startTime" type="text" placeholder="请输入开始时间" style="height: 30px;line-height: 30px;"/>
					<span class="input-group-addon">
						<i class="icon-calendar"></i>
					</span>
			</div>
		    <label class="title" style="float: left;height: 30px;line-height: 30px;width:23px;">到</label>
		   <div class="input-group input-group-sm" style="float: left;width: 234px;margin-right:20px;height: 30px;line-height: 30px;">
				<input class="form-control" id="endTime" type="text" placeholder="请输入结束时间" style="height: 30px;line-height: 30px;"/>
					<span class="input-group-addon">
						<i class="icon-calendar"></i>
					</span>
			</div>
		   <label class="title">事项名称：</label>
                                  ...
                                  <div class="widget-body">
									...
									<label class="title col-xs-4" id="mark_audit"></label>
                                        ...
                                        <div class="add-item-btn" id="detilbtns" style="margin-left:0px;">
                                          <a style="width: 100px;" class="add-itemBtn" onclick="getdetils(); ">调度单明细</a></div>
                                      </div>
                                      ...
                                      <div id="portCost_msg">
                                        ...
                                        <hr class="tree"></hr>
															<div class="add-item col-xs-12">
																...
																<label class="title col-xs-4" id="balanceCash_port"></label>
															...
															<div id="needmoney">
																...
																<label class="title col-xs-2">实付现金：</label> 
																	<label class="title col-xs-4" id="balanceCash_need"></label>
																	<label class="title col-xs-2">实付油费：</label> 
																	<label class="title col-xs-4" id="balanceOil_need"></label> 
                                        ...
                                        <div class="add-item-btn" id="portCost_detilbtn" style="margin-left:0px;">
                                          <a class="add-itemBtn" onclick="getportCostdetils();" style="width:130px;">装费用核算申请明细</a></div>
                                      </div>
                                      ...
                                      <div class="add-item col-xs-12" style="margin-bottom: 10px;" id="hidden8">
                                        ...
                                        <div class="col-xs-4">
                                          <select class="form-control" id="newcarNumber_audit">
                                            <option value="">请选择装运车号</option></select>
                                        </div>
                                        <label class="title col-xs-2">新驾驶员：</label>
                                        <div class="col-xs-4">
...
function init(){
	...
	var myTable = $('#detailtable').DataTable({
		 ...
		 "sAjaxSource": "${ctx}/dailyOffice/waitingDo/getAllListData" , //获取数据的ajax方法的URL							 
		 ordering: false,	
			"oLanguage": {
				...
				columns: [{ data: "rownum","width":"6%"},
						    ...
						    {data: "receiveUserName","width":"16%"},
						    {data: "insertTime","width":"13%"},
						    {data: null,"width":"13%"},
						    {data: "insertUserName","width":"13%"},
						    {data: "processDetailName","width":"13%"},
						    {data: null,"width":"10%"}],
		    columnDefs: [
				{
					//时间
					 ...
					 render: function (data, type, row, meta) {
						 if(row.departmentName!=''&&row.departmentName!=null){
							 return '['+row.departmentName+']  '+data;
						 }else{
							 return data;
						 }
				       }	       
				...
				},
				{
					//时间
					 targets:4,
					 render: function (data, type, row, meta) {
						 var s = row.insertTime;
						 var now = new Date();
						 var c = now.getTime()-s;
						 return MillisecondToDate(c);
				       }	       
				},
		      	{
			    	 //操作栏
			    	 targets: 7,
			    	 render: function (data, type, row, meta) {
			    			 return '<a class="table-edit" onclick="doshow('+ row.id +','+row.processDetailId+','+row.itemId+')">查看</a>';
		                }	       
		    	} 
		      ],
...
/* 查看信息 */
function doshow(id,processDetailId,itemId){
	/* alert(id);
	alert(processDetailId);
	alert(itemId); */
	opendetil(itemId,processDetailId,id);
}
...
/* 	  bootbox.dialog({
		message: '<iframe frameborder="0" src="${ctx}/dailyOffice/waitingDo/detail"/ style="width: 100%;height: 100%;">',
        title: "Custom title",
        className: "my-modal",
        buttons: {
        success: {
