...
<div class="page-content">
	...
	<div class="page-content">
		<div class="searchbox col-xs-12">
		   ...
		   <select id="fom_mainId" class="form-box" >	</select>
				   ...
				   <div>
					  <div class="widget-box dia-widget-box">
							...
							<div class="add-item extra-itemSec">
							    ...
							    <select id="mainId" class="form-control" >	</select>
							     <input class="form-control" id="id-hidden" type="hidden"/>
							  </div>							 							
							  <hr class="tree"></hr>
							  <div class="add-item extra-itemSec">
							     <label class="title"><span class="red">*</span>比率(%)：</label>
							     <input class="form-control" id="ratio" type="text" placeholder="请输入比率 "/>
							  </div>							 			  
							    <hr class="tree"></hr>
...
function init(){
	...
	var myTable = $('#detailtable').DataTable({
		 ...
		 "sAjaxSource": "${ctx}/basicSetting/oilCardRatioMng/getListData" , //获取数据的ajax方法的URL							 
		 ordering: false,	
			"oLanguage": {
...
function BindOutSour(){
	$.ajax({
		type : 'POST',
		url : "${ctx}/basicSetting/oilCardRatioMng/getOutSourcingList",
		contentType : "application/json;charset=UTF-8",
		dataType : 'JSON',
		success : function(data) {
			if (data && data.code == 200) {
				var html ='<option value="">请选择收款方</option>';
...
function dodelete(id){
	...
	bootbox.confirm({ 
		  size: "small",
		  message: "确定要删除该油卡结算比率信息?", 
		  callback: function(result){
			  if(result){
				  $.ajax({
						type : 'GET',
						url : "${ctx}/basicSetting/oilCardRatioMng/delete/"+id,
						contentType : "application/json;charset=UTF-8",
						dataType : 'JSON',
						success : function(data) {
							if (data && data.code == 200) {
...
function save(){
	...
	if(ratio==''|| ratio==null){
		bootbox.alert('比率不能为空！');
		return;
	}if(ratio!=''&&isNaN(ratio)){
		bootbox.alert('比率请填写数字！');
		return;
	}	
	bootbox.confirm({ 
		  size: "small",
		  message: "确定要保存该新增油卡结算比率信息?", 
		  callback: function(result){
			  if(result){
					$.ajax({
						type : 'POST',
						url : '${ctx}/basicSetting/oilCardRatioMng/save',
						data : JSON.stringify({
							ratio : ratio,				
							mainId : mainId
						}),
...
function doedit(id){	
	...
	$.ajax({
		type : 'GET',
		url : "${ctx}/basicSetting/oilCardRatioMng/getDetailData/"+id,
		contentType : "application/json;charset=UTF-8",
		dataType : 'JSON',
		success : function(data) {
			if (data && data.code == 200) {
...
function update(){
	...
	bootbox.confirm({ 
		  size: "small",
		  message: "确定要更新该油卡结算比率信息?", 
		  callback: function(result){
			  if(result){
					$.ajax({
						type : 'POST',
						url : '${ctx}/basicSetting/oilCardRatioMng/update',
						data : JSON.stringify({
							id : id,
							mainId : mainId,				
							ratio : ratio
