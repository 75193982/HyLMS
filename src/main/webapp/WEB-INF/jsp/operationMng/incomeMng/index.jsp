...
<div class="page-content">
	...
	<div class="page-content">
		<div class="searchbox col-xs-12">
			<label class="title mul-title" style="width:75px;">供应商：</label>
		    <select id="supplierId" class="form-box mul-form-box" style="width:233px;">
		    </select>
		    <label class="title mul-title" style="width:75px;">品牌：</label>
		    <select id="brand" class="form-box mul-form-box" style="width:233px;">
		    </select>
		    
		</div>
		<div class="searchbox col-xs-12">
		  <label class="title mul-title" style="float: left;height: 34px;line-height: 34px;width:75px;">下单日期：</label>
		    <div class="input-group input-group-sm" style="float: left;width: 234px;margin-right:20px; margin-left: 2px;height: 34px;line-height: 34px;">
				<input class="form-control" id="waybillDate" type="text" placeholder="请输入下单日期" style="height: 34px;line-height: 34px;"/>
					<span class="input-group-addon">
						<i class="icon-calendar"></i>
					</span>
			</div>
		    <label class="title mul-title" style="float: left;height: 34px;line-height: 34px;width:75px;">装运日期：</label>
		    <div class="input-group input-group-sm" style="float: left;width: 234px;margin-right:20px; margin-left: 2px;height: 34px;line-height: 34px;">
		...
		<div class="searchbox col-xs-12">
		   <label class="title mul-title" style="width:75px;">调度单号：</label>
		    <input id="scheduleBillNo" class="form-box mul-form-box" type="text" style="width:233px;" placeholder="请输入调度单号"/>
		    <label class="title mul-title" style="width:75px;">运单状态：</label>
		    <select id="waybillStatus" class="form-box mul-form-box" style="width:233px;">
...
function init(){
	var myTable = $('#detailtable').DataTable({
		 ...
		 "sAjaxSource": "${ctx}/operationMng/incomeMng/getListData" , //获取数据的ajax方法的URL							 
		 "ordering": false,	
			"oLanguage": {
				...
				columns: [{ data: "rownum",'width':'4%'},
						    ...
						    {data: "waybillNo",'width':'5%'},
						    {data: "scheduleDate",'width':'5%'},
						    {data: "scheduleBillNo",'width':'5%'},
						    {data: "startAddress",'width':'5%'},
						    {data: "targetCity",'width':'5%'},
						    {data: "count",'width':'4%'},
						    {data: "distance",'width':'4%'},
						    {data: "price",'width':'4%'},
						    {data: "balancePrice",'width':'4%'},
						    {data: "bargePrice",'width':'4%'},
						    {data: "farePrice",'width':'8%'},
						    {data: "otherDeduct",'width':'8%'},
						    {data: "sumPrice",'width':'5%'},
						    {data: "waybillStatus",'width':'5%'},
							],
						    columnDefs: [
								
								{
									 ...
									 render: function (data, type, row, meta) {
										 return '<p><span style="width:55px;float:left;">'+data+'</span>'
...
function moneySum(){
	   ...
	   $.ajax({
			type : 'POST',
			url : '${ctx}/operationMng/incomeMng/getSumPrice',
			data : JSON.stringify({
...
function showPrice(id,flag){
	...
	$.ajax({
		type : 'POST',
		url : "${ctx}/operationMng/incomeMng/getAttachDetail",
		data : JSON.stringify({
			carStockId : id,
			chargeType :flag
		}),
		...
		bootbox.confirm({ 
			  ...
			  callback: function(result){
				  if(result){
						$.ajax({
							type : 'POST',
							url : "${ctx}/operationMng/incomeMng/save",
							data : JSON.stringify({
...
function doprint(){
	   ...
	   $.ajax({
		    type : 'POST',
			url : "${ctx}/operationMng/incomeMng/getPrint",
			data : JSON.stringify({
				supplierId : supplierId,
				brand : brand,
				waybillDate : waybillDate,
					...
					if(data.data.length>0){
						for(var i=0;i<data.data.length;i++){
							    data.data[i]["rownum"]=i+1;
							     ...
							     if(data.data[i]["supplierName"]==null || data.data[i]["supplierName"]==''){
