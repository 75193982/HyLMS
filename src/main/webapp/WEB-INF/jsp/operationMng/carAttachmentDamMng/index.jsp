...
<div class="page-content">
	...
	<div class="page-content">
		<div class="searchbox col-xs-12">
		    <!-- <label>入库单号： </label> <input type="text" id="waybill" name="waybill">  -->
		...
		<table id="dynamic-table" class="table table-striped table-bordered table-hover">
			...
			<div class="modal fade" id="modal-info" tabindex="-1" role="dialog" data-backdrop="static">
				<div class="modal-header">
					...
					<h3 id="myModalLabel">折损配件入库登记</h3>
				</div>
...
function loadTable(){
	$('#dynamic-table').DataTable( {
		"destroy": true,//如果需要重新加载的时候请加上这个
		 ...
		 "sAjaxSource": "${ctx}/operationMng/carAttachmentDamMng/getListData" , //获取数据的ajax方法的URL							 
		 ordering: false,	
			"oLanguage": {
		 ...
		 columns: [{ data: "rownum" },
		    ...
		    },
		    {
		        ...
		        render: function(data, type, row, meta) {
		             ...
		             else {return '<a class="table-edit" onclick="doedit('+ row.id +','+row.status+')">查看</a>'};
		        }
		    }
		    ],
...
$.ajax({  
    url: '${ctx}/operationMng/carAttachmentDamMng/getWaybillList',  
    type: "post",  
    contentType : "application/json;charset=UTF-8",
	dataType : 'JSON',
    data: '',
...
function clear()
{
	...
	$("#count").val('');
	$("#position").val('');
	//$('#waybill2').find("option[value='-1']").attr("selected",true);
	$("#mark").val('');
}
//保存
function save()
	...
	bootbox.confirm({
		...
		callback: function(result){
			  if(result){
				  $.ajax({
	            		type : 'POST',
	            		url : "${ctx}/operationMng/carAttachmentDamMng/save",
	            		data : JSON.stringify({		            			
...
function doedit(id,status)
{
	...
	$.ajax({
		type : 'POST',
		url : "${ctx}/operationMng/carAttachmentDamMng/getCarAttachmentList",	
		data:{id:id},
		dataType : 'JSON',
		success : function(data) {
			if (data && data.code == 200) 
    ...
    bootbox.confirm({ 
		  ...
		  callback: function(result){
			  if(result){
				  $.ajax({
						type : 'POST',
						url : "${ctx}/operationMng/carAttachmentDamMng/update",
						data : JSON.stringify({
							id:id,
							attachmentName : attachmentName,
							count : count,
	...
	bootbox.confirm({
		...
		callback:function(result){
			if(result)
			{
				...
				$.ajax({
					type:'post',
					url : "${ctx}/operationMng/carAttachmentDamMng/delete",
            		data : {id:id},
            		//contentType : "application/json;charset=UTF-8",
            		dataType : 'JSON',
            		success : function(data){
