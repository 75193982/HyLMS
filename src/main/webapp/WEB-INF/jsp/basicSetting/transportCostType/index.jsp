				...
				<div class="modal-dialog" style="padding-top:6%;">
				  <div class="modal-content">
				     <div class="modal-header" style="padding:0 15px;">
						...
						<h3 id="myModalLabel">新增报销费用类型信息</h3>
				    </div>
...
function init(){
	...
	var myTable = $('#detailtable').DataTable({
		 ...
		 "sAjaxSource": "${ctx}/basicSetting/transportCostType/getListData" , //获取数据的ajax方法的URL							 
		 "ordering": false,	
			"oLanguage": {
...
function dodelete(id){
	...
	bootbox.confirm({ 
		  size: "small",
		  message: "确定要删除该报销费用类型信息?", 
		  callback: function(result){
			  if(result){
				  $.ajax({
						type : 'GET',
						url : "${ctx}/basicSetting/transportCostType/delete/"+id,
						data :{},
						dataType : 'JSON',
						success : function(data) {
							if (data && data.code == 200) {
...
function save(){
	...
	bootbox.confirm({ 
		  size: "small",
		  message: "确定要保存该报销费用类型信息吗?", 
		  callback: function(result){
			  if(result){
					$.ajax({
						type : 'POST',
						url : '${ctx}/basicSetting/transportCostType/save',
						data : JSON.stringify({
							name :name,
							mark :mark,
							type :''
...
function doedit(id){
	...
	$.ajax({
		type : 'GET',
		url : "${ctx}/basicSetting/transportCostType/getById/"+id,
		data :{},
		dataType : 'JSON',
		success : function(data) {
			if (data && data.code == 200) {
