...
<div class="page-content">
	...
	<div class="page-content">
		<div class="searchbox col-xs-12">
		   ...
		   <input id="fom_outSourcing" class="form-box" type="text" style="width: 110px" placeholder="请填写承运商"/>
		   <input id="fom_outSourcing_id" class="form-box" type="hidden" style="width: 240px" />
		 
			<label class="title">库&nbsp;&nbsp;&nbsp;区：</label>
			<select id="stockSearch" class="form-box" style="width: 110px"><option value="">请选择库区</option>	</select>
			<label class="title">品&nbsp;&nbsp;&nbsp;&nbsp;牌：</label>
			<select id="brandNameSearch" class="form-box" style="width: 110px"><option value="">请选择品牌</option>	</select>
		   <label class="title">车&nbsp;&nbsp;&nbsp;型：</label>
		   <input id="carTypeSearch" class="form-box" type="text" style="width: 110px" placeholder="请填写车型"/>
		</div>
		<div class="searchbox col-xs-12">
		   <label class="title">供应商：</label>
		   <input id="supplierNameSearch" class="form-box" type="text" style="width: 110px" placeholder="请填写供应商"/>
			<label class="title">始发地：</label>
		   <input id="startAddressSearch" class="form-box" type="text" style="width: 110px" placeholder="请填写始发地"/>
		   <label class="title">目的省：</label>
		   <input id="endProvinceSearch" class="form-box" type="text" style="width: 110px" placeholder="请填目的省"/>
		    <label class="title">目的地：</label>
		   <input id="endAddressSearch" class="form-box" type="text" style="width: 110px" placeholder="请填写目的地"/>
							...
							<div class="widget-body">
								<div class="widget-main">
								   <div class="add-item extra-item">
								     ...
								     <select id="outSourcingName" class="form-control" ></select>
								    </div>
								    <hr class="tree"></hr>
								    <div class="add-item extra-item">
					    ...
					    <div class="widget-box dia-widget-box">
							<div class="widget-body">
								<div class="widget-main">
								   <div class="add-item extra-item">
								     <label class="title"><span class="red">*</span>承运商：</label>
								     <select id="eoutSourcingName" class="form-control" ></select>
								    </div>
								    <hr class="tree"></hr>
								    <div class="add-item extra-item">
									     <label class="title"><span class="red">*</span>供应商：</label>
...
function clickp(e){
	$('#fom_outSourcing').val($(e).html());
	$('#fom_outSourcing_id').val($(e).attr('sid'));
	$('#selectItem').hide();
	var outSourcingId = $('#fom_outSourcing_id').val();
	if(outSourcingId != ''){
		 $.ajax({
			type : 'POST',
			url : "${ctx}/basicSetting/outSourcingBusinessPrice/getLibsForOutSourcing/",
			contentType : "application/json;charset=UTF-8",
			dataType : 'JSON',
			data : JSON.stringify({outSourcingId : outSourcingId}),
			success : function(data) {
				if (data && data.code == 200) {
					...
					if(data.data!=null && data.data!=''){
		        		if(data.data.length>0){
		        			for(var i=0;i<data.data.length;i++){
		            			html +='<option value='+data.data[i]['libName']+'>'+data.data[i]['libName']+'</option>';
		            		}
		        		}
		        	}
		 ...
		 $.ajax({
				type : 'POST',
				url : "${ctx}/basicSetting/outSourcingBusinessPrice/getBrandNameForQueryOutSourcing",
				contentType : "application/json;charset=UTF-8",
				data : JSON.stringify({outSourcingId : outSourcingId}),
				dataType : 'JSON',
				success : function(data) {
					if (data && data.code == 200) {
						...
						if(data.data!=null && data.data!=''){
			        		if(data.data.length>0){
			        			for(var i=0;i<data.data.length;i++){
			            			html +='<option value='+data.data['brandName']+'>'+data.data[i]['brandName']+'</option>';
			            		}
			        		}
			        	}
...
function init(){
	...
	var myTable = $('#detailtable').DataTable({
		 ...
		 "sAjaxSource": "${ctx}/basicSetting/outSourcingBusinessPrice/getListData" , //获取数据的ajax方法的URL							 
		 "ordering": false,	
			"oLanguage": {
...
$(function(){
	...
	getOutSourcing();
	getSupplier();
	$("#fom_outSourcing").selectSupplier("#selectItem");
})
...
function getOutSourcing(){
	$.ajax({
		type : 'POST',
		url : "${ctx}/basicSetting/outSourcingBusinessPrice/getOutSourcingList",
		contentType : "application/json;charset=UTF-8",
		dataType : 'JSON',
		success : function(data) {
			if (data && data.code == 200) {
				...
				$('#outSourcingName').html(html);
				$('#eoutSourcingName').html(html);
			} else {
				bootbox.alert(data.msg);
			}
		}
...
function getSupplier(){
	$.ajax({
		type : 'POST',
		url : "${ctx}/basicSetting/outSourcingBusinessPrice/getSupplierList",
		contentType : "application/json;charset=UTF-8",
		dataType : 'JSON',
		success : function(data) {
			if (data && data.code == 200) {
...
function getbrand(e,index){
	...
	if(id=='-1'){
		...
		$.ajax({
			type : 'POST',
			url : "${ctx}/basicSetting/outSourcingBusinessPrice/getAllBrandsForOutSourcing",
 ...
 function getbrandForOutSourcing(id,brandName){
	if(id=='-1'){
		var htmlEm ='<option value="-1">请选择品牌</option>';
		$('#ebrandName').html(htmlEm);
		$('#ebrandName').val('-1');
	}else{
		$.ajax({
	...
	bootbox.confirm({ 
		  ...
		  callback: function(result){
			  if(result){
				  $.ajax({
						type : 'GET',
						url : "${ctx}/basicSetting/outSourcingBusinessPrice/delete/"+id,
						data :{},
						dataType : 'JSON',
						success : function(data) {
							if (data && data.code == 200) {
...
function save(){
	...
	var outSourcingId=$('#outSourcingName').val();
	var brandName=$('#brandName').val();
	var supplierId = $('#supplierName').val();
	if(outSourcingId=='-1' || outSourcingId == '' || outSourcingId == null){
		bootbox.alert('承运商不能为空！');
		return;
	}
	if(supplierId=='-1' || supplierId == '' || supplierId == null){
		bootbox.alert('供应商不能为空！');
		return;
	}
	if(brandName=='-1' || brandName == '' || brandName == null){
	...
	bootbox.confirm({ 
		  ...
		  callback: function(result){
			  if(result){
					$.ajax({
						type : 'POST',
						url : '${ctx}/basicSetting/outSourcingBusinessPrice/save',
						data : JSON.stringify({
							id : '',
							outSourcingId :outSourcingId,
...
function doedit(id){
	...
	$.ajax({
		type : 'GET',
		url : "${ctx}/basicSetting/outSourcingBusinessPrice/getDetailData/"+id,
		data :{},
		dataType : 'JSON',
		success : function(data) {
			if (data && data.code == 200) {
				if(data.data.outSourcingId!=null && data.data.outSourcingId!=''){
					$('#eoutSourcingName').val(data.data.outSourcingId);
				}else{
					$('#eoutSourcingName').val('-1');
				}
				$('#esupplierName').val(data.data.supplierId);
				if(data.data.supplierId!=null && data.data.supplierId!=''){
					getbrandForOutSourcing(data.data.supplierId,data.data.brandName);
				}else{
					$('#ebrandName').val('-1');
				}
...
function update(){
	...
	var outSourcingId=$('#eoutSourcingName').val();
	var brandName=$('#ebrandName').val();
