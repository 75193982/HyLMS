...
<div class="page-content">
	...
	<div class="page-content">
		<div class="searchbox col-xs-12">
		   ...
		   <input id="brandName" class="form-box" type="text" placeholder="请填写品牌"/>
...
function init(){
	...
	var myTable = $('#detailtable').DataTable({
		 ...
		 "sAjaxSource": "${ctx}/basicSetting/carBrandMng/getListData" , //获取数据的ajax方法的URL							 
		 ordering: false,	
			"oLanguage": {
		 ...
		 columns: [{ data: "rownum" ,"width":"4%"},
		    {data: "brandName","width":"14%","class":"wordwear"},
		    {data: "carType","width":"14%","class":"wordwear"},
		    {data: "mark","width":"14%","class":"wordwear"},
		    {data: "insertTime","width":"14%"},
		    {data: "insertUser","width":"14%"},		    
		    {data: null,"width":"14%"}],
		    columnDefs: [
	...
	bootbox.confirm({ 
		  ...
		  callback: function(result){
			  if(result){
				  $.ajax({
						type : 'GET',
						url : "${ctx}/basicSetting/carBrandMng/delete/"+id,
						contentType : "application/json;charset=UTF-8",
						dataType : 'JSON',
						success : function(data) {
							if (data && data.code == 200) {
...
function doedit(id){	
	...
	$.ajax({
		type : 'GET',
		url : "${ctx}/basicSetting/carBrandMng/getDetailData/"+id,
		data :JSON.stringify({
			id :id
		}),
		contentType : "application/json;charset=UTF-8",
		...
		success : function(data) {
			if (data && data.code == 200) {
				...
				$('#brandName_new').val(data.data.brandName);
				$('#carType_new').val(data.data.carType);
				$('#mark_new').val(data.data.mark);				
				$('#addBtn').hide();
				$('#editBtn').show();
				$('#modal-info').modal('show');
	...
	bootbox.confirm({ 
		  ...
		  callback: function(result){
			  if(result){
					$.ajax({
						type : 'POST',
						url : '${ctx}/basicSetting/carBrandMng/update',
						data : JSON.stringify({
							id : id,
							brandName : brandName,				
							carType : carType,
