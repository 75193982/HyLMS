...
<div class="page-content">
	...
	<div class="page-content">
		<div class="searchbox col-xs-12">
			<label class="title" style="float: left;">承运商：</label>
		...
		<table id="detailtable" class="table table-striped table-bordered table-hover">
				...
				<div class="modal-dialog" style="padding:0;margin:auto;">
				  <div class="modal-content" style="border:0;">
				     <div class="modal-header" style="padding:0 15px;">
					  ...
					  <h3 id="myModalLabel">新增承运商往来款信息</h3>
				     </div>
			...
			<div class="modal fade modal_car" id="modal-edit" tabindex="-1" role="dialog"  data-backdrop="static">
				<div class="modal-dialog" style="padding:0;margin:auto;">
				  <div class="modal-content" style="border:0;">
				     <div class="modal-header" style="padding:0 15px;">
					  <button class="close" type="button" data-dismiss="modal">×</button>
					  <h3 id="myModalLabel">编辑承运商往来款信息</h3>
				     </div>
					<div class="modal-body" style="padding:5px 20px;">
						  <div class="widget-box dia-widget-box">
							<div class="widget-body">
		 ...
		 <div class="modal fade" id="modal-view" tabindex="-1" role="dialog" data-backdrop="static">
				<div class="modal-header">
					<button class="close" type="button" data-dismiss="modal" >×</button>
					<h3 id="myModalLabel" style="margin:5px;">查看承运商往来款信息</h3>
				</div>
...
function init(){
	var myTable = $('#detailtable').DataTable({
		 ...
		 "sAjaxSource": "${ctx}/reportMng/payableFund/getListData" , //获取数据的ajax方法的URL							 
		 "ordering": false,	
			"oLanguage": {
...
function bindbusiness(){
	$.ajax({
		type : 'POST',
		url : "${ctx}/reportMng/payableFund/getOutSourcingList",
		contentType : "application/json;charset=UTF-8",
		dataType : 'JSON',
		success : function(data) {
			if (data && data.code == 200) {
	...
	bootbox.confirm({ 
		  ...
		  callback: function(result){
			  if(result){
					$.ajax({
						type : 'POST',
						url : '${ctx}/reportMng/payableFund/save',
						data : JSON.stringify({
							id:0,
...
function doedit(id)
{
	...
	$.ajax({
		type : 'GET',
		url : "${ctx}/reportMng/payableFund/getDetailData/"+id,
		contentType : "application/json;charset=UTF-8",
		dataType : 'JSON',
		success : function(data) {
			if (data && data.code == 200) {
...
function dosubmit(id)
{
		...
		bootbox.confirm({ 
			  size: "small",
			  message: "确定要提交该承运商往来款信息?", 
			  callback: function(result){
				  if(result){
					  $.ajax({
							type : 'GET',
							url : "${ctx}/reportMng/payableFund/submit/"+id,
							dataType : 'JSON',
							success : function(data) {
								if (data && data.code == 200) {
									 bootbox.confirm_alert({ 
...
function dodelete(id)
{
	...
	bootbox.confirm({ 
		  size: "small",
		  message: "确定要删除该承运商往来款信息?", 
		  callback: function(result){
			  if(result){
				  $.ajax({
						type : 'GET',
						url : "${ctx}/reportMng/payableFund/delete/"+id,
						dataType : 'JSON',
						success : function(data) {
							if (data && data.code == 200) {
								 bootbox.confirm_alert({ 
...
function printInfo(){
    ...
    /*    var day = date.getDate() < 10 ? "0" + date.getDate() : date.getDate();
	   ...
	   $.ajax({
		    type : 'POST',
			url : "${ctx}/reportMng/payableFund/getPrintData",
			data : JSON.stringify({
				fundDateStart : $.trim($('#fundDateStart').val()),
				fundDateEnd :$.trim($('#fundDateEnd').val()),
				businessId :$.trim($('#fom_business').val()),
						...
						for(var i=0;i<data.data.length;i++){
							...
							if(data.data[i]["fundType"]==null || data.data[i]["fundType"]==''){
								...
								data.data[i]["fundTypeName"]=''; 
								 ...
								 data.data[i]["fundTypeName"]='应付款';  
							 }else{
								 data.data[i]["fundTypeName"]='实付款';  
							 }
...
function exportInfo()
{
	   ...
	   var form = $('<form action="${ctx}/reportMng/payableFund/getExportData" method="POST"></form>');
