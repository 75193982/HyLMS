...
<div class="page-content">
	...
	<div class="page-content">
		<div class="searchbox col-xs-12">
		    ...
		    <input id="fom_name" class="form-box" type="text" style="width: 240px" placeholder="请填写供应商"/>
				   ...
				   <div>
					  <div class="widget-box dia-widget-box">
							...
							<div class="add-item">
							     ...
							     <div class="form-new">
							     <input id="name" name="name" type="text" class="form-control" placeholder="请填写供应商" />
							 ...
							 <div class="add-item">
							     ...
							     <div class="form-new">
							     <input id="address" class="form-control" type="text" placeholder="请填写地址"/>
							     </div>
							 </div>
							 <hr class="tree"></hr>
							 <div class="add-item">
							     <label class="title"><span class="red">*</span>联系人：</label>
							     <div class="form-new">
							     <input id="linkUser" class="form-control" type="text" placeholder="请填写联系人"/>
							     </div>
							 </div>
							 <hr class="tree"></hr>
							  <div class="add-item">
							     ...
							     <div class="form-new">
							     <input id="linkTelephone" class="form-control" type="text" placeholder="请填写联系电话"/>
							     </div>
							 </div>
							 <hr class="tree"></hr>
							 <div class="add-item">
							     ...
							     <div class="form-new">
							     <input id="linkMobile" class="form-control" type="text" placeholder="请填写手机号码"/>
							     </div>
							 </div>
							 <hr class="tree"></hr>
							  <div class="add-item">
							  ...
							  <!-- <hr class="tree"></hr>
							   <div class="add-item">
							     <label class="title"><span class="red">*</span>供应商库 ：</label>
							     <div class="form-new"> 
							       <input id="stocks" class="form-control" type="text" placeholder="请填写供应商库,多个以‘|’隔开"/>
							       </div>
							  </div>	 -->
							 <hr class="tree"></hr>
							  <div class="add-item">
							     <label class="title">合同开始时间 ：</label>
							     <div class="input-group input-group-sm form-new" style="width:400px;">
							   ...
							   <hr class="tree"></hr>
							  <div class="add-item">
							     <label class="title">合同结束时间 ：</label>
							     <div class="input-group input-group-sm form-new" style="width:400px;">
							   ...
							   <hr class="tree"></hr>
							    <div class="add-item-btn" id="addBtn">
								 ...
								 <div class="add-item-btn" id="editBtn">
								    <a class="add-itemBtn btnOk" onclick="update()" style="margin-left: 130px;">保存</a>
								    <a class="add-itemBtn btnOk" onclick="refresh()">关闭</a>
								  </div> 								  
								</div>
						  </div>
...
function init(){
	...
	var myTable = $('#detailtable').DataTable({
		 ...
		 "sAjaxSource": "${ctx}/basicSetting/supplierMng/getListData" , //获取数据的ajax方法的URL							 
		 ordering: false,	
			"oLanguage": {
		 ...
		 columns: [{ data: "rownum" ,"width":"8%"},
		    ...
		    {data: "linkUser","width":"10%"},
		    {data: "linkTelephone","width":"10%"},
		    {data: "linkMobile","width":"10%"},
		    ...
		    columnDefs: [{
				 ...
				 render: function (data, type, row, meta) {
			    	 ...
			    	 targets: 9,
			    	 render: function (data, type, row, meta) {
			    		 return '<a class="table-bigbtn" style="width:62px;" onclick="dobrand('+ row.id +')">品牌管理</a>'
		    			   +'<a class="table-edit" style="width:35px;" onclick="doedit('+ row.id +')">编辑</a>'
				           +'<a class="table-delete" style="width:35px;" onclick="dodelete('+ row.id +')">删除</a>';	 
...
function retrieveData( sSource, aoData, fnCallback ) {
	   ...
	   var needInvoiceFlag=$("#fom_needInvoiceFlag").val(); 
	   var name=$("#fom_name").val(); 
	   var invoiceOrder=$("#fom_invoiceOrder").val(); 
	   $('#secho').val(secho);
	   var obj = {};
		 $.ajax({
			type : 'POST',
			...
			data : JSON.stringify({
				...
				name : $.trim(name),
				needInvoiceFlag : $.trim(needInvoiceFlag),
				invoiceOrder : $.trim(invoiceOrder)
			}),
			contentType : "application/json;charset=UTF-8",
			dataType : 'JSON',
			success : function(data) {
	...
	bootbox.confirm({ 
		  ...
		  callback: function(result){
			  if(result){
				  $.ajax({
						type : 'GET',
						url : "${ctx}/basicSetting/supplierMng/delete/"+id,
						contentType : "application/json;charset=UTF-8",
						dataType : 'JSON',
						success : function(data) {
							if (data && data.code == 200) {
...
function save(){
	...
	if(linkTelephone==''|| linkTelephone==null){
		bootbox.alert('联系电话不能为空！');
		return;
	}
	/* if(stocks==''|| stocks==null){
		bootbox.alert('供应商库不能为空！');
		return;
	} */
	bootbox.confirm({ 
		  size: "small",
		  ...
		  callback: function(result){
			  if(result){
					$.ajax({
						type : 'POST',
						url : '${ctx}/basicSetting/supplierMng/save',
						data : JSON.stringify({
							name : $.trim(name),
...
function doedit(id){	
	...
	$.ajax({
		type : 'GET',
		url : "${ctx}/basicSetting/supplierMng/getDetailData/"+id,
		contentType : "application/json;charset=UTF-8",
		dataType : 'JSON',
		success : function(data) {
			if (data && data.code == 200) {
	...
	bootbox.confirm({ 
		  ...
		  callback: function(result){
			  if(result){
					$.ajax({
						type : 'POST',
						url : '${ctx}/basicSetting/supplierMng/update',
						data : JSON.stringify({
							id : id,
							name : $.trim(name),
				  			address : $.trim(address),
...
/*设置结算价格*/
function dobrand(id){
	location.href="${ctx}/basicSetting/supplierBusiness/index/"+id;
}
