...
//"香港特别行政区"  ['中西区', '湾仔区', '东区', '南区', '油尖旺区', '深水埗区', '九龙城区', '黄大仙区', '观塘区', '荃湾区', '葵青区', '沙田区', '西贡区', '大埔区', '北区', '元朗区', '屯门区', '离岛区' ],
//"澳门特别行政区"  ['澳门']

//省份
var provinces = ["---请选择省份---","北京市","天津市","上海市","重庆市","河北省","河南省","云南省","辽宁省","黑龙江省","湖南省","安徽省","山东省","新疆维吾尔自治区","江苏省","浙江省","江西省","湖北省","广西壮族自治区","甘肃省","山西省","内蒙古自治区","陕西省","吉林省","福建省","贵州省","广东省","青海省","西藏自治区","四川省","宁夏回族自治区","海南省"];
//城市
var cities = [
		['---请选择市县---'],
...
function getElemtnt(param)
{
		//window["province"+i];//定义变量
		//window["city"+i];//定义变量
		province=document.getElementById("startProvince"+param);  
		city=document.getElementById("startAddress"+param); 
		endProvince = document.getElementById("endProvince"+param);
		endCity = document.getElementById("endAddress"+param);
...
function createProvincesEnd(){  
    for(var i in provinces){  
        //获取下拉列表集合实例  
        var op=new Option(provinces[i],provinces[i]);  
        //将下拉列表各元素添加到省份元素中  
        endProvince.options.add(op);
    }  
}
//编辑后根据省加载城市 
function loadcitys(param,city){
	var city3 = document.getElementById(city);
	for(var i in provinces)
	{
		if(provinces[i] == param)
		{
			for(var j in cities[i])
	    	{
	    		var op=new Option(cities[i][j],cities[i][j]); 
	            city3.options.add(op); 
	    	}
		}
	}
} 
...
//编辑后根据省加载城市 
function loadcitysEnd(param,city){
	var city3 = document.getElementById(city);
	for(var i in provinces)
	{
		if(provinces[i] == param)
...
function createCities(par,param){ 
    //获取省份选中项的索引  
    var index=$(par).prop('selectedIndex');//.selectedIndex; 
    //将city下的所有原有option移除  
    var city2 = document.getElementById(param);
...
function createCitiesEnd(par,param){ 
	var index=$(par).prop('selectedIndex');
	var city2 = document.getElementById(param);
    city2.options.length=0;  
    for(var i in cities[index]){  
        var op=new Option(cities[index][i],cities[index][i]);  
