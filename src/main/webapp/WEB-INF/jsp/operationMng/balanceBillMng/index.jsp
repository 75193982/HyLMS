...
<div class="page-content">
	...
	<div class="page-content">
		<div class="searchbox col-xs-12">
		   ...
		   <div class="input-group input-group-sm" style="float: left;width: 234px;margin-right:19px;margin-left: 2px;">
		...
		<div class="searchbox col-xs-12">
		    <label class="title">运单编号：</label>
		     <input id="form_waybillNo" class="form-box" type="text" placeholder="请输入运单编号" style="width:511px;"/>
		    <label class="title" style="width:70px;">状态：</label>
		    <select id="form_status" class="form-box">
...
function init(){
	...
	var myTable = $('#detailtable').DataTable({
		 ...
		 "sAjaxSource": "${ctx}/operationMng/balanceBillMng/getListData" , //获取数据的ajax方法的URL							 
		 ordering: false,	
			"oLanguage": {
	...
	bootbox.confirm({ 
		  ...
		  callback: function(result){
			  if(result){
				  $.ajax({
						type : 'GET',
						url : "${ctx}/operationMng/balanceBillMng/sure/"+id,
						dataType : 'JSON',
						success : function(data) {
							if (data && data.code == 200) {
...
function doshow(e){
	...
	$.ajax({
		type : 'GET',
		url : "${ctx}/operationMng/balanceBillMng/getDetailPrintData/"+id,
		dataType : 'JSON',
		success : function(data) {
			if (data && data.code == 200) {
				$('#id-hidden').val(id);				
...
function doedit(e){
	...
	$.ajax({
		type : 'GET',
		url : "${ctx}/operationMng/balanceBillMng/getDetailData/"+id,
		dataType : 'JSON',
		success : function(data) {
			if (data && data.code == 200) {
				$('#id-hidden').val(id);
...
function changenumb(e,type){
	...
	$.ajax({
		type : 'POST',
		url : "${ctx}/operationMng/balanceBillMng/getAmount",
		data : JSON.stringify({
			id : $('#id-hidden').val(),				
	...
	bootbox.confirm({ 
		  ...
		  callback: function(result){
			  if(result){
					$.ajax({
						type : 'POST',
						url : '${ctx}/operationMng/balanceBillMng/update',
						data : JSON.stringify({
							id:id,
							carCount : carCount,
							distance : distance,
	   ...
	   bootbox.confirm({ 
			  ...
			  callback: function(result){
				  if(result){
					  $.ajax({
						    type : 'POST',
							url : "${ctx}/operationMng/balanceBillMng/getPrint",
							data : JSON.stringify({
		  ...
		  callback: function(result){
			  if(result){
				  var waybillNo=$('#form_waybillNo').val();
				   ...
				   var endTime=$('#form_endTime').val();
				   var status=$('#form_status').val();
				   var form = $('<form action="${ctx}/operationMng/balanceBillMng/export" method="post"></form>');
