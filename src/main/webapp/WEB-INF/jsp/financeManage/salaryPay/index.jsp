	  ...
	  <div class="detailInfo p5">
	     <div class="searchbox col-xs-12">
               <label class="title" style="float: left;height: 34px;line-height: 34px;">工资归属时间：</label>
				...
				<a class="itemBtn" id="saveSchedule" onclick="addSarlar()">新增</a> 
					...
					<table class="easyui-datagrid" id="dg1" style="width:100%;height:100%" data-options="fitColumns:true,singleSelect:true,onLoadSuccess:onLoadSuccess,onClickRow:getDetail">   
					    <thead>   
					        <tr>  
					            ...
					            <th data-options="field:'userCount',width:'8%',align:'center'">人数</th>
					            <th data-options="field:'amount',width:'15%',align:'center'">发放总金额</th>  
					            <th data-options="field:'status',width:'6%',align:'center',formatter:formatterStaus">状态</th>   
					            <th data-options="field:'updateTime',width:'23%',align:'center'">操作时间</th> 
					            <th data-options="field:'updateUserName',width:'10%',align:'center'">操作人</th>
					            <th data-options="field:'id',width:'16%',formatter:formatOper">操作</th>    
					        </tr>   
					    </thead>   
					</table> 
				</td>
...
<script type="text/javascript">
	...
	function formatOper(val,row,index){
		if(row.status=='0'){
			return '<a href="#" id="editSarlar" style="width:66px;margin:5px;" onclick="editDetail('+row.id+')">编辑</a>'
				  +'<a href="#" id="editSarlar" style="width:66px;margin:5px;" onclick="deleteDetail('+row.id+')">删除</a>';
  ...
  function searchInfo(){
   ...
   /* 新增 */
	function addSarlar(){
		location.href="${ctx}/financeManage/salaryPay/new/0/N";
   }
  /* 编辑 */
  function editDetail(id){
	 var ids=parseInt(id);
	 location.href="${ctx}/financeManage/salaryPay/new/"+ids+"/N";
  }
  /* 删除 */
		...
		bootbox.confirm({ 
			  ...
			  callback: function(result){
				  if(result){
					  $.ajax({
							type : 'GET',
							url : "${ctx}/financeManage/salaryPay/delete/"+id,
							data : '',
							dataType : 'JSON',
							success: function(data){
								if (data && data.code == 200){
